...
module type GlMatT = {
  let load_identity: unit => unit;
  let mode: [ | `modelview | `projection | `texture] => unit;
};
...
module type GlDrawT = {
  ...
  let begins: shape => unit;
  let color: alpha::float? => rgb => unit;
  let vertex2: point2 => unit;
  let ends: unit => unit;
};

module type GlutT = {
  let init: argv::array string => array string;
  let initWindowSize: w::int => h::int => unit;
  let initDisplayMode:
    double_buffer::bool? =>
    index::bool? =>
    accum::bool? =>
    ...
    unit =>
    unit;
  let createWindow: title::string => int;
  type button_t =
    | LEFT_BUTTON
    | MIDDLE_BUTTON
    | RIGHT_BUTTON
  ...
  type mouse_button_state_t =
    ...
    | UP;
  let mouseFunc:
    cb::(button::button_t => state::mouse_button_state_t => x::int => y::int => unit) => unit;
  let passiveMotionFunc: cb::(x::int => y::int => unit) => unit;
  let displayFunc: cb::(unit => unit) => unit;
  let idleFunc: cb::option (unit => unit) => unit;
  let mainLoop: unit => unit;
  let postRedisplay: unit => unit;
  let swapBuffers: unit => unit;
};

module type GlClearT = {
  let clear : list [`color] => unit;
};

module type GlStuffT = {
  let module GlDraw: GlDrawT;
  let module Glut: GlutT;
  let module GlMat: GlMatT;
  let module GlClear: GlClearT;
};

let module Make (M: GlStuffT) => {
  let module Glut = M.Glut;
  let module GlDraw = M.GlDraw;
  let module GlClear = M.GlClear;
  let module GlMat = M.GlMat;
  ...
  let start () => {
    ...
    Glut.initWindowSize w::windowSize h::windowSize;
